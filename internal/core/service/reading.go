...
func (s *ReadingService) Search(ctx context.Context, world, scope, query string) (domain.Document, error) {
	raw, err := s.world.Lookup(ctx, domain.LookupRequest{World: world, Scope: scope, Query: query})
	if err != nil {
		return domain.Document{}, err
	}
...
func (s *ReadingService) Tag(ctx context.Context, world, tag string) (domain.Document, error) {
	raw, err := s.world.Lookup(ctx, domain.LookupRequest{World: world, Scope: "/", Query: tag, Filter: "tag=" + tag})
	if err != nil {
		return domain.Document{}, err
	}
