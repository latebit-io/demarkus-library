...
`DEMARKUS_BRANDING` / `DEMARKUS_THEME_CSS` / `DEMARKUS_LOGO` to the named
keys. `name` alone works without any ConfigMap. Rolling a rebrand is operator-driven, matching the
